...
Recent work explores LLM-based multi-agent systems for improved reasoning. \citet{du2023debate} demonstrate that multi-agent debate improves factuality and mathematical reasoning, with agents proposing and debating responses over multiple rounds. \citet{yazici2026consensus} study opinion consensus formation among networked LLMs, applying classical consensus models to predict group behavior. Our work differs by providing a \emph{standardized protocol} for consensus rather than ad-hoc debate frameworks.

\citet{ruan2025agreement} address the challenge of reaching agreement among reasoning LLM agents, while \citet{wu2024debate} provide a controlled study of multi-agent debate in logical reasoning. The recent LatentMAS framework \cite{zou2025latentmas} enables communication through latent representations rather than text, achieving 14.6\% accuracy gains with 70-83\% token reduction for same-architecture models.
...
For verification, \citet{chen2025toolmad} propose Tool-MAD, combining multi-agent debate with tool augmentation for fact verification. \citet{he2025debatetruth} introduce DebateCV for claim verification through structured debate. We provide a more general verification protocol applicable to any claim type.

\subsection{Uncertainty Quantification}

Epistemic uncertainty in LLMs remains challenging. \citet{nel2025kalshibench} evaluate calibration via prediction markets, finding models often overconfident. \citet{li2025esi} propose semantic-preserving interventions for uncertainty quantification. Our epistemic classification takes a different approach: rather than quantifying confidence on a continuum, we categorize questions by their \emph{answerability structure}.

\subsection{Model Routing and Selection}

Intelligent model selection has emerged as a practical concern given the proliferation of specialized models. \citet{khan2025art} present ART, using tournament-style ELO ranking for response optimization. Our routing mechanism differs by maintaining explicit capability profiles per model and task type, enabling predictive selection before query execution.
...
\begin{tabular}{lcccc}
\toprule
Method & Factual & Reasoning & Verification & Avg \\
\midrule
Single (best) & 78.3 & 81.2 & 72.4 & 77.3 \\
Self-consistency & 82.1 & 86.4 & 74.8 & 81.1 \\
Multi-agent debate & 83.7 & 87.9 & 79.2 & 83.6 \\
\textbf{MCIP (ours)} & \textbf{85.2} & \textbf{89.1} & \textbf{84.6} & \textbf{86.3} \\
\bottomrule
\end{tabular}
\caption{Accuracy (\%) across task categories}
\label{tab:results}
\end{table}

\subsection{Consensus Level vs. Accuracy}

A key finding is that consensus level strongly predicts accuracy:
...
\midrule
HIGH ($\geq 85\%$) & 47.3\% & 94.2\% \\
MEDIUM (60-84\%) & 31.2\% & 82.7\% \\
LOW (30-59\%) & 14.8\% & 68.4\% \\
NONE/CONTRADICTORY & 6.7\% & 51.2\% \\
\bottomrule
\end{tabular}
\caption{Consensus level as accuracy predictor}
\end{table}
...
On our custom hallucination benchmark (1000 claims designed to elicit confident false responses):

\begin{itemize}
    \item Single model: 34.7\% hallucination rate
    \item Multi-model consensus: 18.2\% (-47.5\%)
    \item With verification: 11.4\% (-67.1\%)
\end{itemize}
...
We presented MCIP, a protocol for multi-model consensus and inference that addresses fundamental limitations of single-model AI systems. By enabling models to query, verify, and challenge each other, we achieve 23-47\% reduction in hallucinations and 94.2\% accuracy on high-consensus queries. The epistemic classification framework prevents wasted computation on unanswerable questions and provides users with actionable confidence signals.
...
\newblock \emph{arXiv preprint arXiv:2601.xxxxx}, 2026.
...
\newblock \emph{arXiv preprint arXiv:2512.xxxxx}, 2025.
...
\newblock \emph{arXiv preprint arXiv:2411.xxxxx}, 2025.
...
\newblock \emph{arXiv preprint arXiv:2507.xxxxx}, 2025.
