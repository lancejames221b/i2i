...
%   Cross-list: cs.CL (Computation and Language), cs.MA (Multiagent Systems)
%
% This paper presents AI system architecture and multi-agent coordination
% protocols, making cs.AI the appropriate primary category.
...
\begin{abstract}
Large Language Models (LLMs) demonstrate remarkable capabilities but suffer from hallucinations, single-model biases, and inability to express epistemic uncertainty. We present \textbf{i2i} and the \textbf{Multi-model Consensus and Inference Protocol (MCIP)}, a standardized framework for AI-to-AI communication that addresses these limitations through multi-model consensus, cross-verification, epistemic classification, and intelligent routing. Our key insight is not that consensus universally improves accuracy, but that \emph{consensus level reliably predicts answer trustworthiness}. In evaluation across 400 questions spanning factual QA, hallucination detection, mathematical reasoning, and commonsense tasks, we find that HIGH consensus ($\geq$85\% agreement) achieves \textbf{92.6\% overall accuracy} (97.8\% on non-mathematical tasks where consensus is appropriate). We demonstrate that consensus provides a 6\% improvement in hallucination detection (38\%$\rightarrow$44\%), with LOW/NONE consensus reliably flagging confabulated answers. Critically, we also show that consensus \emph{degrades} performance on mathematical reasoning (95\%$\rightarrow$60\% on GSM8K), revealing that different reasoning chains should not be averaged---a finding that informs task-appropriate deployment. We introduce \emph{epistemic classification} to distinguish answerable questions from uncertain, underdetermined, or ``idle'' questions. The protocol is provider-agnostic, supporting OpenAI, Anthropic, Google, xAI, and local models. Code and specification: \url{https://github.com/lancejames221b/i2i}.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Consensus Mechanism}: Algorithms for detecting agreement levels (HIGH/MEDIUM/LOW/NONE/CONTRADICTORY) across model responses with empirically validated reliability.
...
Figure~\ref{fig:architecture} illustrates the MCIP architecture. The \texttt{AICP} class serves as the primary orchestrator, delegating to specialized engines for consensus, verification, epistemic classification, and routing. The \texttt{ProviderRegistry} abstracts over heterogeneous AI providers, enabling provider-agnostic operations.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    node distance=0.8cm and 1.2cm,
    box/.style={rectangle, draw, rounded corners, minimum width=2.2cm, minimum height=0.8cm, font=\small, fill=white},
    engine/.style={box, fill=blue!10},
    provider/.style={box, fill=green!10, minimum width=1.6cm},
    flow/.style={-{Stealth[length=2mm]}, thick},
    dashed_flow/.style={-{Stealth[length=2mm]}, thick, dashed},
    label_style/.style={font=\scriptsize\itshape, text=gray}
]

% Main AICP node
\node[box, fill=yellow!20, minimum width=3cm, minimum height=1cm, font=\small\bfseries] (aicp) {AICP Protocol};

% Engines row
\node[engine, below left=1.2cm and 1.5cm of aicp] (consensus) {Consensus\\Engine};
\node[engine, right=0.4cm of consensus] (verify) {Verification\\Engine};
\node[engine, right=0.4cm of verify] (epistemic) {Epistemic\\Classifier};
\node[engine, right=0.4cm of epistemic] (router) {Model\\Router};

% Provider Registry
\node[box, fill=orange!15, below=1.2cm of verify, minimum width=5cm] (registry) {Provider Registry};

% Providers row
\node[provider, below left=0.8cm and 1.5cm of registry] (openai) {OpenAI};
\node[provider, right=0.3cm of openai] (anthropic) {Anthropic};
\node[provider, right=0.3cm of anthropic] (google) {Google};
\node[provider, right=0.3cm of google] (xai) {xAI};
\node[provider, right=0.3cm of xai] (local) {Ollama};

% User Query
\node[above=0.8cm of aicp, font=\small] (query) {User Query};

% Response
\node[below=3.8cm of aicp, font=\small] (response) {ConsensusResult / VerificationResult / EpistemicClassification};

% Arrows from AICP to engines
\draw[flow] (aicp.south) -- ++(0,-0.3) -| (consensus.north);
\draw[flow] (aicp.south) -- ++(0,-0.3) -| (verify.north);
\draw[flow] (aicp.south) -- ++(0,-0.3) -| (epistemic.north);
\draw[flow] (aicp.south) -- ++(0,-0.3) -| (router.north);

% Arrows from engines to registry
\draw[flow] (consensus.south) -- ++(0,-0.3) -| ($(registry.north)+(-1,0)$);
\draw[flow] (verify.south) -- (verify.south |- registry.north);
\draw[flow] (epistemic.south) -- (epistemic.south |- registry.north);
\draw[flow] (router.south) -- ++(0,-0.3) -| ($(registry.north)+(1,0)$);

% Arrows from registry to providers
\draw[flow] (registry.south) -- ++(0,-0.2) -| (openai.north);
\draw[flow] (registry.south) -- ++(0,-0.2) -| (anthropic.north);
\draw[flow] (registry.south) -- ++(0,-0.2) -| (google.north);
\draw[flow] (registry.south) -- ++(0,-0.2) -| (xai.north);
\draw[flow] (registry.south) -- ++(0,-0.2) -| (local.north);

% Query arrow
\draw[flow] (query) -- (aicp);

% Response arrow
\draw[flow] ($(local.south)+(0,-0.3)$) -- (response);

% Labels
\node[label_style, right=0.1cm of aicp.east] {orchestrator};
\node[label_style, below=0.05cm of registry.south east, anchor=north east] {abstraction layer};

\end{tikzpicture}
\caption{MCIP Architecture. User queries flow through the AICP orchestrator to specialized engines, which coordinate with multiple AI providers via the Provider Registry.}
...
Figure~\ref{fig:epistemic_tree} illustrates the decision process for epistemic classification.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    node distance=0.6cm,
    decision/.style={diamond, draw, fill=yellow!20, text width=2cm, text centered, inner sep=1pt, font=\scriptsize},
    outcome/.style={rectangle, draw, rounded corners, fill=green!20, text width=1.8cm, text centered, minimum height=0.6cm, font=\scriptsize\bfseries},
    outcome_bad/.style={rectangle, draw, rounded corners, fill=red!20, text width=1.8cm, text centered, minimum height=0.6cm, font=\scriptsize\bfseries},
    outcome_warn/.style={rectangle, draw, rounded corners, fill=orange!20, text width=1.8cm, text centered, minimum height=0.6cm, font=\scriptsize\bfseries},
    arrow/.style={-{Stealth[length=1.5mm]}, thick},
    label_yes/.style={font=\tiny, text=green!60!black},
    label_no/.style={font=\tiny, text=red!60!black}
]

% Root question
\node[decision] (coherent) {Logically coherent?};

% Level 1
\node[outcome_bad, right=1.5cm of coherent] (malformed) {MALFORMED};
\node[decision, below=0.8cm of coherent] (evidence) {Evidence available?};

% Level 2
\node[decision, below left=0.8cm and 0.5cm of evidence] (certain) {Single answer?};
\node[decision, below right=0.8cm and 0.5cm of evidence] (actionable) {Actionable?};

% Level 3 outcomes
\node[outcome, below left=0.8cm and -0.3cm of certain] (answerable) {ANSWERABLE};
\node[outcome_warn, below right=0.8cm and -0.3cm of certain] (uncertain) {UNCERTAIN};
\node[outcome_warn, below left=0.8cm and -0.3cm of actionable] (underdet) {UNDER-\\DETERMINED};
\node[outcome_bad, below right=0.8cm and -0.3cm of actionable] (idle) {IDLE};

% Arrows
\draw[arrow] (coherent) -- node[label_no, above] {no} (malformed);
\draw[arrow] (coherent) -- node[label_yes, right] {yes} (evidence);
\draw[arrow] (evidence) -- node[label_yes, above left] {yes} (certain);
\draw[arrow] (evidence) -- node[label_no, above right] {no} (actionable);
\draw[arrow] (certain) -- node[label_yes, left] {yes} (answerable);
\draw[arrow] (certain) -- node[label_no, right] {no} (uncertain);
\draw[arrow] (actionable) -- node[label_yes, left] {yes} (underdet);
\draw[arrow] (actionable) -- node[label_no, right] {no} (idle);

\end{tikzpicture}
\caption{Epistemic classification decision tree. Questions are classified by logical coherence, evidence availability, determinacy, and actionability.}
\label{fig:epistemic_tree}
\end{figure}

\subsection{The ``Idle Question'' Concept}

The IDLE classification captures questions that are ``well-formed but idle''---coherent grammatically but their answers do not guide any action.
...
\textbf{Note}: This section describes the routing architecture implemented in i2i. Systematic evaluation of routing effectiveness is planned for future work; results in Section~\ref{sec:eval} focus on consensus mechanisms.
...
For factual knowledge tasks (TriviaQA, TruthfulQA), consensus either improves or maintains accuracy. The key benefit is \emph{calibration}: HIGH consensus questions achieve 97.8\% accuracy on non-mathematical tasks (92.6\% overall), providing a reliable trust signal.
...
Figure~\ref{fig:consensus_accuracy} visualizes the relationship between consensus level and accuracy. The clear monotonic relationship demonstrates that consensus level serves as a reliable proxy for answer trustworthiness.
...
\begin{axis}[
    ...
    symbolic x coords={HIGH, MEDIUM, LOW, NONE},
    xtick=data,
    ymin=0, ymax=105,
...
\legend{All benchmarks, Factual only}
\end{axis}
\end{tikzpicture}
\caption{Consensus level strongly predicts accuracy. HIGH consensus ($\geq$85\% agreement) achieves 92.6\% accuracy overall and 97.8\% on factual tasks. The monotonic relationship enables confidence-aware deployment.}
...
\begin{itemize}
    \item HIGH consensus ($\geq$85\% agreement) achieves 92.6\% overall accuracy (97.8\% on factual/verification tasks)
